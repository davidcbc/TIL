...
class AVLNode(BinaryNode):
    ...
    def add(self, new_value):
        super(AVLNode, self).add(new_value)
    ...
    def delete(self, value):
        if super(AVLNode, self).delete(value) is True:
            if self is None:
                ...
                self.left_depth = 0 if self.left_node is None else max(self.left_node.left_depth,
                                                                       self.left_node.right_depth) + 1
                self.right_depth = 0 if self.right_node is None else max(self.right_node.left_depth,
                                                                         self.right_node.right_depth) + 1
                self.balance = self.left_depth - self.right_depth
                if self.balance is 2 and self.left_node is not None and self.left_node.balance is -1:
                    self.left_node.rotate_left()
                    self.left_node.parent = self
